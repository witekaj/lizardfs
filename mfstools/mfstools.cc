...
int main(int argc,char **argv) {
    ...
    boost::algorithm::erase_all( cmdName, "/");
    ...
    if ( "lizardfstools" == cmdName || "mfstools" == cmdName ) {
        if (argc==2 && strcmp(argv[1],"create")==0) {
            fprintf(stderr,"create symlinks\n");
            LizardTool::createFsLink( "mfstools");
            return 0;
        } else {
            lizardCmd->showHelpMsg();
            return 1;
        }
    }

    f = lizardCmd->getFunctionEnum().first;
    rflag = lizardCmd->getFunctionEnum().second;
...
{
    LIZARD_TOOLS_MAP.insert( make_pair( pTool->getToolName(), pTool ) );
    return true;
}
